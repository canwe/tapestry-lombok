...
public class Guess {
	...
	/*
	 * public Object onActivate() { if (target == 0) target = random.nextInt(10) + 1; if (conversationId == null ||
	 * !conversationManager.exists(conversationId)) { conversationId =
	 * conversationManager.createConversation(componentResources.getPageName(), 10, true); return null; } else return
	 * null; }
	 */
	...
	/*
	 * String onPassivate() { return null; // conversationId; }
	 */
	...
	@OnEvent("guess")
	Object onGuess(int guess) {
		if (guess == target) return gameEndBlock;

		if (guess < target) message = String.format("%d is too low.", guess);
		else message = String.format("%d is too high.", guess);
