...
@Import(library="DynamicSelect.js")
/**
...
public class ChildSelect extends AbstractField {
	...
	/**
	 * A list models available to this select. The size of this list
	 * should be equal to the size of the parent list.
	 */
	...
	protected void processSubmission(String elementName) {
		...
		Object selectedValue = InternalUtils.isBlank(submittedValue) ? null
				: getEncoder().toValue(submittedValue);

	
	    value =  ((OptionModel)selectedValue).getValue();
	...
	void afterRender(MarkupWriter writer) {
		...
		for (int i = 0; i < models.size(); i++) {
			...
			jsSupport.addScript(generateJS(selectModel, arrayId),
					new Object[0]);
			...
			String str = selectModelId + "=new SelectionModel(" + arrayId
					+ ",new OptionModel(\"" + indexOfDefaultOption
					+ "\",\"" + defaultOption.getLabel() + "\"));";

			jsSupport.addScript(str, new Object[0]);
		...
		modelsSB.append("];");
		jsSupport.addScript(modelsSB.toString(), new Object[0]);
		String controllerStr = "new ChildListController(\"" + parentId
				+ "\",\"" + childId + "\"," + modelsId + ").setChildList();";
		jsSupport.addScript(controllerStr, new Object[0]);
	}
	...
	void options(MarkupWriter writer) {
		...
		SelectModel model = new SelectModelImpl(null,
	 ...
	 * @param selectModel
	 * @param varName
	 * @return A javascript to create an array of OptionModel in selectModel.
	 * Eg var name=[new OptionModel("0", "X"), new OptionModel("1", "Y")];
	 */
	protected String generateJS(SelectModel selectModel,
			String varName) {
		StringBuffer sb = new StringBuffer("var " + varName + "=[");
		List<OptionModel> options = selectModel.getOptions();

		for (int i = 0; i < options.size(); i++) {
			OptionModel optionModel = options.get(i);
			sb.append("new OptionModel(\"" + i + "\",\""
	...
	/**
	 * 
	 * @param options A list of OptionModels
	 * @param value 
